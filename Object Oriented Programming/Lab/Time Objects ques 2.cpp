...
void time::compare(class time b){
	...
	for(int i = 0; i<5; i++){
		if(time_taken[i]>b.time_taken[i]){
			...
			cout<<b.name<<" takes just "<<time_taken[i]<<" hours to do the task"<<i+1<<" but "<<name<<" takes "<<b.time_taken[i]<<" hours to do the same task"<<endl;
			cout<<endl;
		}
		else if(time_taken[i]<b.time_taken[i]){
			cout<<name<<" does the task"<<i+1<<" faster than "<<b.name<<" ."<<endl;
			cout<<name<<" takes just "<<b.time_taken[i]<<" hours to do the task"<<i+1<<" but "<<b.name<<" takes "<<time_taken[i]<<" hours to do the same task"<<endl;
			cout<<endl;
		}
		else{
